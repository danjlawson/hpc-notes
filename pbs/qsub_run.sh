...
function show_help {
    ...
    echo "  -j <val>: Set the number of processes run in parallel (-j command of parallel). (Default: same as -n)"
...
function makefile {
    t_cmdfile=`basename ${cmdfile}`
    pbsfile="$dir/${t_cmdfile}_cmd${filenum}.pbs"
    echo "#!/bin/bash
#PBS -l walltime=$walltime:00:00,nodes=1:ppn=$nodes
#PBS -o $dir/${t_cmdfile}_cmd${filenum}.log
#PBS -e $dir/${t_cmdfile}_cmd${filenum}.err
" > $curdir/$pbsfile
...
function makepfile {
    t_cmdfile=`basename ${cmdfile}`
    pbsfile="$dir/${t_cmdfile}_cmd${filenum}.pbs"
    parallelfile="$dir/${t_cmdfile}_cmd${filenum}.parallel"
    rm -f $parallelfile
    touch $parallelfile
    echo "Making $parallelfile"
    echo "mkdir $parallelfile.d" >> $pbsfile
    echo "cat $parallelfile | $parallel --tmpdir $parallelfile.d -j $parallelnodes" >> $pbsfile
}
...
parallel="parallel"
pretend=0
while getopts "h?vpP:n:j:q:s:w:m:f:" opt; do
    case "$opt" in
    f) cmdfile=$OPTARG
	;;
...
dir=`mktemp  -d --tmpdir=. tmp_${dir}XXX`
#dir="tmp_$dir"
mkdir -p "$dir"
